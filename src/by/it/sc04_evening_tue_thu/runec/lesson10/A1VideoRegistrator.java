    ...
    List<Double> calcStartTimes(double[] events, double workDuration){
        ...
        Arrays.sort(events);                  //подготовка к жадному поглощению массива событий
         ...
         while (i<events.length)   {          //пока есть незарегистрированные события
          ...
          double stop=start+workDuration;     //и запомним время старта видеокамеры
          result.add(start);
          while (i<events.length&&events[i]<=stop) //вычислим момент окончания работы видеокамеры
           i++; }                                  //и теперь пропустим все покрываемые
