package by.it.sc04_evening_tue_thu.tokun.lesson10;
...
public class B1Sheduler {
    ...
    List<Event> calcStartTimes(Event[] events, int from, int to) {
        ...
        for (int i = 0; i < events.length; i++) {
            Event lastResult = result.get(result.size() - 1);
            if(lastResult.getStop() <= events[i].getStart()
                    && events[i].getStop() <= to) result.add(events[i]);
        }
