...
public class A_BinaryFind {
    int[] findIndex(InputStream stream) throws FileNotFoundException {
        ...
        for (int i = 0; i < k; i++) {
            ...
            int low=a[0];
            int high=a.length-1;result[i]=-1;
            while (low<=high) {
                int mid=(low+high)/2;
                if (a[mid]==value) {result[i]=mid+1; break;}
                else if (a[mid]<value) {
                    low=mid+1;
                }
                else high=mid-1;
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        InputStream stream = new FileInputStream(root + "by/it/sc03_evening_mon_fri/mironchik/lesson11/dataA.txt");
