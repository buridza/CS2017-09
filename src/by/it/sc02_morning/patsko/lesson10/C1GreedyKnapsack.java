...
public class C1GreedyKnapsack {
    ...
    double calc(File source) throws FileNotFoundException {
        ...
        int vesVRance=0;
        for (int i = 0; ; i++) {
            if((vesVRance+items[i].weight)<W){
                vesVRance+=items[i].weight;
                result+=items[i].cost;
            }
            else{ result+=items[i].cost/items[i].weight*(W-vesVRance); break;}
        }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        File f=new File(root+"by/it/sc02_morning/patsko/lesson10/greedyKnapsack.txt");
